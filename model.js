        ...
        { //预读本地硬盘的用户信息
            ...
            if(str){
             let users = JSON.parse(str) ;
              Model.users = users ;
              for(let w of Model.users){
                let y=2022+Math.floor(4*Math.random());
                let m=Math.floor(12*Math.random());
                let d=Math.floor(28*Math.random());
                w.timer=new Date(y+'/'+m+'/'+d);
                if(Math.random()<0.5){
                    let level;
                    w.level=0;
                }
 ...
 let UI = {} ; //UI用于表达用户界面，以及改变用户界面上的的内容
   UI.printWord = function(){ //用于把当前单词（Model.pos存储的索引）显示出来
       //让全局变量与局部变量联系起来
        ...
        let s = "" ;
        if (Model.users[pos].timer){
          let d = Model.users[pos].timer ;
  ...
  UI.footerLog = function(s){
    ...
    setTimeout(() => {
        select('footer').textContent = "江西科技师范大学  2024--2025" ;
    }, 3*1000);
  };
